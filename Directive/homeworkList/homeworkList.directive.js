...
angular.module("app").directive("homeworkList",function(){
    return {
        templateUrl: "./Directive/homeworkList/homeworkList.html?v=1918",
        restrict:"EA",
        scope:{
            "chooseCallback":"=",
            "showAddBtn":"=",
        ...
        controller: function ($http, $scope, enume, $state, $rootScope, $stateParams, ngDialog) {
            ...
            $scope.createHomework = function(){
                if($rootScope.userFlag == "userSafe"){
                    $state.go("safeRoom.homeworkCreate", { entity: { tag: "add", from: "safe", teachingCode: "20161101205908881NAN2", schoolID: "BJ1001",Type:$scope.Type } });
                }else{
                    $state.go("safeRoom.homeworkCreate", { entity: { tag: "add", from: "root" } });
                }
            }

            $scope.getUrl = function(){
                return srvDomain + "/Homework/Index?homeworkTitle=" + $scope.Title + "&courseTitle=" + $scope.CourseTitle + "&status=" + $scope.statusID + "&type=" + $scope.Type;
            }
            ...
            $scope.detail = function (item) {
                $scope.showButton = true;
                $state.go("safeRoom.homeworkCreate", { entity: { tag: "detail", homeworkID: item.ID ,Type:$scope.Type } });
            }
